...
@ConditionalOnProperty(
        value = "application.spring-batch.jobs.integration-health-check-job.schedule.enabled",
        havingValue = "true"
)
@Slf4j
...
public class IntegrationHealthCheckScheduler {
     ...
     */
    @Scheduled(
            cron = "${application.spring-batch.jobs.integration-health-check-job.schedule.cron}",
            zone = "${application.spring-batch.jobs.integration-health-check-job.schedule.zone}"
    )
    public void tick() {
